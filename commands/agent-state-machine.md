...
```jsonl
{"timestamp": "2024-01-01T10:00:00", "type": "session_init", "state": "MAIN", "permissions": {"can_write": ["*.md", "*.txt", "*.log", "*.json", "*.csv"], "cannot_write": ["*.py", "*.js", "*.ts", "*.sh", "*.go", "*.rs", "*.c", "*.cpp", "*.java", "package.json", "requirements.txt", "Dockerfile", "Makefile", "any executable file"], "can_execute": ["read", "list", "search", "analyze", "and get info function"],"cannot_execute": ["run", "exec", "eval", "compile", "build"]}, "parameters": {"mode": "orchestrator", "path": "[directory_path]"}, "agent_descriptions": "/root/.claude/data/AGENT_DIRECTORY.md"}
...
Show transitions: [STATE_A → STATE_B]
Log all transitions to state.jsonl
```
